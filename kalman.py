...
def calculate_initial_mean(prices):
    return np.mean(prices)

def calculate_initial_var(prices):
    return (np.std(prices))**2

def calculate_process_var():
    return 0.001 

def calculate_sensor_var(prices):
    return (np.std(prices))**2

def kalman_filter(close_prices):
    """
    Takes in np.array of close prices of a security
    """
